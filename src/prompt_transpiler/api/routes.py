...
from prompt_transpiler.api.errors import make_error_response
from prompt_transpiler.api.schemas import (
...
from prompt_transpiler.jobs.models import JobRecord, JobStatus
...
def register_routes(app: APIFlask) -> None:
    ...
    def version() -> dict[str, str]:
        return {
            ...
            "environment": current_app.config.get("PROMPT_TRANSPILER_ENV", "dev"),
        }

    @app.post("/v1/transpile-jobs")
    @app.post("/v1/compile-jobs")
    @app.input(CompileRequestSchema)
    @app.output(CompileJobEnqueueResponseSchema, status_code=202)
    @app.doc(
        tags=["transpile", "jobs"],
        summary="Enqueue a transpile job",
        description=(
            "Submit a transpile request and receive a job ID immediately. "
    ...
    def enqueue_compile_job(json_data: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "status_url": f"{base_url}/v1/transpile-jobs/{job_id}",
            "result_url": f"{base_url}/v1/transpile-jobs/{job_id}/result",
        }

    @app.get("/v1/transpile-jobs/<job_id>")
    ...
    @app.get("/v1/transpile-jobs/<job_id>/result")
    @app.get("/v1/compile-jobs/<job_id>/result")
    @app.output(CompileJobResultResponseSchema, status_code=200)
    @app.doc(
        tags=["jobs", "transpile"],
        summary="Get job result",
        description="Fetch the transpile result if the job has completed successfully.",
        responses={
    ...
    def get_job_result(job_id: str) -> tuple[dict[str, Any], int] | dict[str, Any]:
        ...
        if status == JobStatus.SUCCEEDED.value:
            return {
                "job_id": job_id,
                "status": status,
                "result": _hydrate_transpile_result(job.get("result")),
            }
    ...
    @app.delete("/v1/transpile-jobs/<job_id>")
            ...
            "algorithms": [
                {
                    "name": "pairwise",
                    "description": (
                        "Pairwise comparison scoring over baseline vs candidate verdicts."
                    ),
                },
                {
                    "name": "weighted",
...
def _hydrate_transpile_result(result: dict[str, Any] | None) -> dict[str, Any] | None:
...
_hydrate_compile_result = _hydrate_transpile_result
